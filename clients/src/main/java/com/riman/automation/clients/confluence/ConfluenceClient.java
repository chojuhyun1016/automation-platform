 ...
 * HTTP 요청 전송과 응답 반환만 담당하며, 페이지 계층 구성과 보고서 포맷은 상위 계층이 결정한다.
 *
 * PUT은 BaseHttpClient가 미지원하므로 JDK HttpURLConnection으로 직접 구현한다.
 * 인증은 Jira와 동일한 Atlassian 계정을 BasicTokenProvider로 공유한다.
 */
@Slf4j
public class ConfluenceClient extends BaseHttpClient {
  ...
  /**
   * 부모 페이지의 직접 자식 목록에서 title로 페이지 ID를 조회한다.
   *
   * children API를 채택한 이유: Confluence Cloud에서 title+spaceKey 검색의 ancestors 필드가
   * 빈 배열로 반환되는 경우가 있어 부모 매칭이 불안정하기 때문이다.
   * parentPageId가 null이면 space 전체 title 검색으로 폴백한다.
   *
  ...
  /**
   * 부모 페이지의 직접 자식 중 title 일치 페이지 ID를 조회한다.
   *
   * 탐색 3단계:
   *   1. CQL parent={id} AND title="..." AND space=...  (인덱싱 지연으로 누락 가능)
   *   2. title 검색 + ancestors 직접 검증  (인덱싱 지연 없음, 가장 안정적)
   *   3. children API  (최후 수단)
   *
   * @return pageId, 없으면 null
   */
  ...
  /**
   * title + spaceKey 검색 후 ancestors 직접 조회로 부모를 검증한다.
   *
   * CQL parent= 조건과 children API 모두 Confluence Cloud에서 인덱싱 지연으로
   * 방금 생성된 페이지를 누락하는 버그가 있다.
   * 반면 title 검색은 인덱싱 지연 없이 즉시 반영된다.
   * 검색 결과의 각 페이지에 대해 expand=ancestors로 직계 부모를 확인하여
   * parentPageId와 일치하는 것만 후보로 수집한다.
   *
   * 중복 페이지 처리 (삭제 없이 올바른 것 선택):
   *   1. 자식 페이지가 있는 것 (= 실제 사용 중인 페이지)
   *   2. 자식 수가 동일하면 id가 낮은 것 (더 먼저 생성된 것)
   *
   * @return pageId (parentPageId의 직계 자식 중 최적), 없으면 null
   */
  public String findPageByTitleAndParent(String parentPageId, String title) {
    try {
      ...
      // 2) ancestors 검증 - 직계 부모가 parentPageId인 것만 수집
  ...
  /**
   * children API 폴백.
   * CQL 실패 시 또는 인덱싱 지연 대응 재탐색 시 사용한다.
   */
   ...
   * 새 페이지를 생성한다.
   *
   * @param parentPageId 부모 페이지 ID (null이면 space 루트)
   * @param title 페이지 제목
  ...
  /**
   * 페이지 내용과 위치(ancestors)를 동시에 업데이트한다.
   * BaseHttpClient는 PUT을 미지원하므로 JDK HttpURLConnection으로 직접 구현한다.
   *
   * @param pageId 업데이트할 페이지 ID
   * @param title 페이지 제목
   * @param storageHtml 새 본문 (Confluence Storage Format)
   * @param newVersion 현재버전 + 1
   * @param parentPageId 이동할 부모 페이지 ID. null이면 위치 변경 없이 내용만 업데이트.
   */
      ...
      try {
        ...
        if (status < 200 || status >= 300) {
          String errBody = conn.getErrorStream() != null
              ...
              : "(no error body)";
          // 5xx/429는 재시도 가능, 4xx(429 제외)는 클라이언트 오류
  ...
  /**
   * 위치 변경 없이 내용만 업데이트한다 (하위 호환 오버로드).
   */
  ...
  /**
   * title 검색 API로 페이지를 찾되, parentPageId의 직접 자식인 것만 반환한다.
   * findChildPageId의 인덱싱 지연 대응 보조 탐색.
   *
  ...
  /**
   * 페이지를 생성하거나 업데이트한다.
   * findChildPageId(3단계 탐색)로 기존 페이지를 검색한다.
   */
  ...
  /**
   * 페이지를 삭제한다.
   * 중복 페이지 정리용. 삭제 실패 시 warn 로그만 남기고 예외를 전파하지 않는다.
   *
  ...
  /**
   * 페이지에 파일을 첨부한다 (multipart/form-data).
   * 같은 이름의 첨부파일이 이미 있으면 update API로 덮어쓰고, 없으면 create API로 신규 첨부한다.
   * (Confluence Cloud는 allowDuplicated=true를 무시하므로 이 방식이 필수)
   *
  ...
  /**
   * 페이지의 첨부파일 목록에서 fileName과 일치하는 첨부파일 ID를 반환한다. 없으면 null.
   *
   * filename 쿼리 파라미터를 사용하지 않는다.
   * URLEncoder.encode()가 공백을 '+'로 인코딩하여 Confluence가 파일을 찾지 못하는 버그가 있다.
   * 대신 전체 목록(최대 50건)을 받아 Java에서 직접 비교한다.
   */
  ...
  /**
   * 재시도 가능 여부를 판정한다.
   * ExternalApiClientException(연결 실패, 타임아웃)과
   * ExternalApiException의 429(Rate Limit) 또는 5xx는 재시도 대상이다.
   * 4xx(429 제외)는 클라이언트 오류이므로 재시도하지 않는다.
   */
