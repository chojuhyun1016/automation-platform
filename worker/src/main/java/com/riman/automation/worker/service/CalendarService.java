...
/**
 * Google Calendar 통합 서비스.
 * Jira 이벤트(CREATE/UPDATE/DELETE), 재택, 부재 이벤트의 Calendar API 호출을 담당한다.
 *
 * Jira 이벤트 조회 전략.
 * 1차: JiraCalendarMappingService의 DynamoDB 매핑 (PK 직접 조회, 빠름).
 * 2차: extendedProperties 기반 스캔 (pre-DynamoDB 이벤트용 하위 호환).
 * 2차 성공 시 DynamoDB에 소급 저장하여 이후 호출부터는 1차에서 해결한다.
 *
 * startdate 처리.
 * 캘린더 이벤트 날짜는 duedate 기준으로만 설정한다 (start=duedate, end=duedate+1일).
 * startdate(customfield_10015)는 캘린더 표시에 반영하지 않고 extendedProperties와 description에만 저장한다.
 */
@Slf4j
public class CalendarService {
  ...
  /** startdate(customfield_10015) 저장 키. 다른 서비스에서 Calendar API로 읽어 참고한다. */
  ...
  /**
   * DynamoDB 매핑 서비스. null이면 DynamoDB 매핑 없이 extendedProperties fallback만 사용한다.
   */
  ...
  /**
   * 테스트용 생성자. Mock 클라이언트와 서비스를 주입할 수 있다.
   */
  ...
  /**
   * 운영용 생성자.
   * S3에서 Google credentials를 로드하고, CALENDAR_MAPPING_TABLE 환경변수가 있으면 DynamoDB 매핑 모드로 초기화한다.
   */
  ...
  /**
   * Jira webhook 이벤트 진입점. webhookEvent에 따라 CREATE/UPDATE/DELETE로 분기한다.
   * Calendar API 429(쿼타 초과) 발생 시 MonitoringAlertService로 알림을 전송한다.
   */
  ...
  /**
   * Jira 이슈 생성 처리.
   * duedate가 없거나 담당자가 비팀원이면 스킵한다.
   * 동일 issueKey의 이벤트가 이미 존재하면 UPDATE로 전환하여 webhook 재전송에 의한 중복 생성을 차단한다.
   */
  private void createJiraEvent(JiraWebhookEvent jiraEvent, ProjectRouting routing) {
    ...
    // webhook 재전송 또는 Lambda 재시도 대비: 이미 존재하면 UPDATE로 전환
  ...
  /**
   * Jira 이슈 업데이트 처리.
   * 이벤트가 없으면 CREATE로 전환, duedate가 제거되면 DELETE로 전환한다.
   * 담당자가 팀원→비팀원으로 바뀌면 기존 팀원 담당자 이름으로 이벤트를 유지 업데이트한다.
   */
  private void updateJiraEvent(JiraWebhookEvent jiraEvent, ProjectRouting routing) {
    ...
    if (currentAssignee == null) {
      // 담당자 비팀원 처리: 이전 팀원 이름을 유지하며 업데이트
  ...
  /**
   * Jira 이슈 삭제 처리. 매핑된 캘린더 이벤트가 있으면 삭제한다.
   */
  ...
  /**
   * 2계층 이벤트 조회.
   * 1차 DynamoDB 매핑 hit, 2차 extendedProperties 스캔 폴백. 2차 성공 시 1차에 소급 저장한다.
   */
  ...
  /**
   * extendedProperties의 jiraIssueKey로 이벤트를 탐색한다 (fallback 전용).
   * extendedProperties가 누락된 옛 이벤트는 summary 접두사 매칭으로 폴백한다.
   */
  ...
  /**
   * "[Jira] CCE-1234 (홍길동)" 형식 summary에서 담당자 이름을 추출한다.
   */
  ...
  /**
   * 특정 날짜의 재택 이벤트("재택(..)" 접두사)를 찾는다. 없으면 null을 반환한다.
   */
  ...
  /**
   * 재택 이벤트 제목만 갱신한다. 이벤트 객체는 listEvents에서 찾아 재사용하여 불필요한 필드 초기화를 피한다.
   */
  ...
  /**
   * 캘린더 이벤트 설명(description)을 구성한다.
   * description은 이벤트 상세보기에서 사람이 확인하는 용도이며, 캘린더 날짜 자체에는 영향이 없다.
   */
  ...
  /**
   * Google Calendar Event 객체를 구성한다.
   * 캘린더 날짜는 duedate 기준(start=duedate, end=duedate+1일)으로 Google all-day 이벤트 규칙을 따른다.
   * startdate는 캘린더 날짜에 반영하지 않고 extendedProperties와 description에만 저장한다.
   */
  ...
  /**
   * startdate를 방어적으로 정규화한다.
   * null 또는 blank이면 오늘 날짜로 대체하고, duedate보다 이후이면 duedate로 보정한다.
   */
  ...
  /**
   * extendedProperties.private에 issueKey/assignee/isTeamMember/startdate를 저장한다.
   */
  ...
  /**
   * S3에서 Google Calendar credentials(JSON)를 로드한다. 필수 환경변수 누락 시 ConfigException.
   */
