 ...
 * 연차 기간은 startDate ~ endDate 각 날짜별로 이벤트를 생성/삭제하며, 취소 시 이벤트가 없으면 DLQ 방지를 위해 조용히 종료한다.
 */
@Slf4j
public class AbsenceService {
  ...
  /**
   * 부재 등록(apply) 또는 취소(cancel)를 처리한다.
   * endDate < startDate면 startDate로 보정한다.
   *
   * @param calendarId 대상 캘린더 ID
   * @param msg        파싱 완료된 AbsenceMessage (한글 이름, endDate 보정 전제)
   */
  ...
  /**
   * 특정 날짜에 부재 이벤트를 생성한다. 동일 이벤트가 이미 있으면 멱등 처리한다.
   */
  ...
  /**
   * 특정 날짜의 부재 이벤트를 삭제한다. 이벤트가 없으면 DLQ 방지를 위해 조용히 종료한다.
   */
  ...
  /**
   * 특정 날짜에서 제목이 "{absenceType}({name})"과 정확히 일치하는 이벤트를 찾는다.
   */
