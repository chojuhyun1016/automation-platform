...
# 수정 모드 (문제 발견 후 재실행):
/resolve-issue 12 특정 API에서 날짜 필터링 안됨
/resolve-issue 12 빌드는 되는데 테스트 실패
```
...
- **빌드 실패 시**: **build-error-resolver 에이전트를 실행**하여 자동 해결. 수동 디버깅하지 마라.
...
- 빌드 검증 — **빌드 실패 시 build-error-resolver 에이전트 실행**
- 관련 테스트 실행
