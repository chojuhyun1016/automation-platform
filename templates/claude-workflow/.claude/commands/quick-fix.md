...
```
# 버그 수정
/quick-fix 특정 API에서 NPE 발생, 로그 첨부...

# 분석만 요청
/quick-fix UserService.findById에서 N+1 쿼리 발생

# 리팩터링
/quick-fix OrderService 메서드가 너무 길다, 분리해줘
...
빌드 에러인 경우: **build-error-resolver 에이전트 즉시 실행** → Step 3으로 건너뛰어도 됨.
...
- **최소 변경 원칙** — 요청된 범위만
- **검증 루프** (빌드 → 테스트 순서로 반복):
  1. 빌드 검증 (CLAUDE.md 빌드 커맨드 참조)
  2. 빌드 실패 시 → **build-error-resolver 에이전트 실행** → 수정 → 1로 복귀
  3. 빌드 성공 시 → 관련 테스트 실행
