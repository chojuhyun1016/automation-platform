...
### SPEC.md Phase 체크박스 완료 규칙

Phase 완료(= 관련 PR 머지) 시 **해당 Phase 섹션의 모든 체크박스**를 `[x]`로 변경할 것:

1. 최상위 `- [x] Phase N 완료 (PR #N)` 체크 (PR 번호 기록)
2. `### 수정/개선` 내 **모든 세부 체크박스** `[x]` (중첩 항목 포함)
3. `### 검증` 내 **모든 체크박스** `[x]`

**누락 시 위험**: 새 Claude 세션이 미완료 체크박스를 보고 "아직 작업 진행 중"으로 오판한다.
누락 방지: `/resolve-issue` 커맨드가 PR 단계에서 자동 강제하며, `/update-docs` 실행 시 PR 상태 기반으로 재동기화한다.

## 변경 원칙

- **최소 변경**: 요청된 범위의 파일만 수정할 것 — 요청하지 않은 리팩터링, 정리, 개선을 포함하지 말 것
- **TDD 적용 기준**: `feat` 브랜치 필수, `fix` 권장, `refactor`/`chore`/`hotfix` 불필요 (skills/tdd-workflow 참조)
