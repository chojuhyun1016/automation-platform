 ...
 *
 * Public Subnet + EIP 방식으로 NAT Gateway 불필요 (비용 절감).
 * ID/PW는 환경변수로 전달하지 않으며, Task 내부에서 Secrets Manager로 직접 조회한다.
 * 컨테이너 이름은 "groupware-bot"으로 Task Definition과 일치해야 한다.
 */
@Slf4j
public class EcsTaskService {
  ...
  /** 컨테이너 이름 — Task Definition의 containerDefinitions[0].name과 일치해야 함. */
