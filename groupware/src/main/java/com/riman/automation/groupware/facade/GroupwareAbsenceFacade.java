 ...
 *
 * 처리 순서:
 *   1. groupware-config.json을 S3에서 로드
 ...
 *
 * cancel 액션은 그룹웨어 자동화가 불가능하므로 Slack 수동 처리 안내만 발송한다.
 */
@Slf4j
public class GroupwareAbsenceFacade {
  ...
  public void handle(GroupwareAbsenceMessage msg) {
    // cancel은 그룹웨어 자동 취소가 불가능하므로 Slack 수동 처리 안내만 발송
    if (!msg.isApply()) {
    ...
    // 5. "처리 중" Slack DM (완료 결과는 Python Task에서 비동기 DM 발송)
   ...
   * ID/PW는 절대 포함하지 않으며, Task 내부에서 GROUPWARE_CREDENTIALS_SECRET으로 직접 조회한다.
   */
