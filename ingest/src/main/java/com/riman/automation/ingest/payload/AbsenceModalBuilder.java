...
/**
 * /부재등록 Slack Modal Block Kit JSON 빌더.
 * callback_id 는 {@code absence_submit} 이며 부재 유형, 등록/취소, 시작일, 종료일, 사유 5개 블록을 구성한다.
 */
public class AbsenceModalBuilder {
  ...
  /**
   * 모달 요청 JSON (trigger_id + view) 전체를 직렬화하여 반환한다.
   */
  ...
  private static ObjectNode buildView(String displayName, String userId, String today) {
    ...
    // private_metadata 에 "userId|displayName" 을 담아 submit 단계에서 복원한다.
