 ...
 * callback_id 는 {@code current_ticket_submit} 이며 조회 기간(daily/weekly/monthly/quarterly) 드롭다운 하나로 구성된다.
 * 기본 선택값은 "분기별"이다.
 */
public class CurrentTicketModalBuilder {
  ...
  private static ObjectNode buildView(String userId) {
    ...
    // 현재티켓은 요청자 본인만 조회하므로 private_metadata 는 userId 1단으로만 구성한다.
    view.put("private_metadata", userId);
