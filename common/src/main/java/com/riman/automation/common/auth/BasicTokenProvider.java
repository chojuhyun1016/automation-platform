...
/**
 * Basic 인증 토큰 제공자.
 * Jira Cloud의 email:apiToken Base64 인코딩 방식에 사용한다.
 */
public class BasicTokenProvider implements TokenProvider {
  ...
  /**
   * 환경변수에서 이메일과 토큰을 읽어 Base64 인코딩한다.
   *
   * @param emailEnvVar 이메일을 담은 환경변수명 (예: JIRA_EMAIL)
   * @param tokenEnvVar 토큰을 담은 환경변수명 (예: JIRA_API_TOKEN)
   */
