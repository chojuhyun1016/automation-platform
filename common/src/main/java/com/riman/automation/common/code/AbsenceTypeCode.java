 ...
 *
 * label은 Slack payload 옵션값, SQS 메시지 absenceType 필드값, 캘린더 이벤트 제목 prefix로 사용된다.
 * singleDayOnly가 true이면 날짜 1개만 허용하며, AbsenceFacade에서 endDate를 startDate로 강제한다.
 *
 * 기간 유형: 연차, 병가, 대체 휴가, 경조 휴가, 포상 휴가, 산전후 휴가, 휴직(관리자등록)
 * 날짜 1개 유형: 오전 반차, 오전 반반차, 오후 반차, 오후 반반차, 보건 휴가, 예비군(민방위) 훈련
 */
@Getter
public enum AbsenceTypeCode {
  ...
  /**
   * label 문자열로 enum 조회. 매칭 없으면 null 반환.
   */
