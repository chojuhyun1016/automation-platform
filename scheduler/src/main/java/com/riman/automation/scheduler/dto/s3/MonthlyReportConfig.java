...
/**
 * 월간 실적 보고 설정 DTO이다. S3 scheduler-config.json의 "monthlyReport" 섹션과 매핑된다.
 * 카테고리 분류는 설정이 아닌 WeeklyReportData.detectCategory 코드 매핑으로 관리되므로 새 프로젝트 추가 시 해당 메서드만 수정한다.
 * Confluence 페이지 계층은 confluence_parent_page_id 하위에 연도 → 분기 → 월간보고 페이지가 자동 생성되며,
 * 보고 기간은 대상 월의 1일부터 말일까지이고 target_month가 없으면 이전 월을 자동 사용한다.
 */
@Data
...
public class MonthlyReportConfig {
  ...
  /**
   * 팀명이다. Confluence 월간보고 페이지 제목 접미사로 사용한다.
   * 예: "보상코어 개발팀" → "2026 Q1 1월 - 보상코어 개발팀 실적". 미설정 시 기본값 "보상코어 개발팀"을 사용한다.
   */
   ...
   * 이벤트 제목 규칙은 "[CCE-123] 제목 (담당자이름)"이며 미설정 시 월간보고 데이터를 수집할 수 없다.
   */
  ...
  /**
   * 월간보고 루트 부모 페이지 ID이다.
   * "실적보고"처럼 이미 존재하는 최상위 페이지 ID이며 이 페이지 하위에 연도/분기/월간보고 계층이 자동 생성된다.
