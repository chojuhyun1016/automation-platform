 ...
 *
 * WeeklyCalendarTicketCollector와 MonthlyCalendarTicketCollector의 중복 코드를
 * 단일 위치로 통합한 stateless 유틸 클래스다. 두 Collector는 이 클래스에 위임한다.
 *
 * 캘린더 이벤트 제목 규칙:
...
public final class CalendarTicketParser {
  ...
  /** 이슈 키 패턴 — "[Jira] CCE-2326" 또는 "[CCE-123]" 형식을 모두 지원한다. */
  ...
  /**
   * 팀원 목록과 매칭되는 첫 번째 이름 반환. 매칭 없으면 이벤트 담당자 첫 번째 이름 반환.
   */
  ...
  /**
   * 이벤트 요약(제목) 추출.
   * description의 "Title: " 라인을 우선 사용, 없으면 캘린더 제목에서 이슈 키/담당자/이모지 제거한다.
   */
   ...
   * 없으면 IN_PROGRESS 반환 — 이후 Jira 재검증 대상이 된다.
   */
  ...
  /**
   * description의 "Priority: " 라인에서 우선순위 감지. 없으면 UNKNOWN 반환.
   */
