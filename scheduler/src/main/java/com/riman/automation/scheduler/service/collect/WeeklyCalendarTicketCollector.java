 ...
 *
 * 캘린더를 원천 데이터로 사용하는 이유:
 * Jira에서 직접 수집하면 티켓 담당자가 타팀원으로 변경된 경우, 우리 팀원이 수행한 작업 이력이 누락된다.
 * Google Calendar는 팀원이 담당했던 이력이 그대로 남으므로 실적 집계에 더 정확하다.
 *
 * 수집 전략 — 캘린더 2회 조회:
 *   1. 완료: 전주(weekStart~weekEnd) 범위 중 Status=DONE인 것
 *   2. 진행중: quarterStart~quarterEnd 범위(분기 전체) 중 Status≠DONE이고
 *      startDate ≤ weekEnd(또는 startDate null)이며 [이슈] 태그가 없는 것
 *   3. 이슈: quarterStart~quarterEnd 범위(분기 전체) 중 [이슈] 태그 포함 미완료이고
 *      startDate ≤ weekEnd(또는 startDate null)인 것
 *
 * 진행중/이슈 startDate 필터 이유:
 * 분기 전체를 조회하면 미래 예정 티켓(startDate > weekEnd)이 포함되므로 제외한다.
