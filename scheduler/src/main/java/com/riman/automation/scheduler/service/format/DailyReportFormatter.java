...
/**
 * 일일 보고서 포맷터 — Slack Block Kit (mrkdwn section)를 생성한다.
 *
 * 들여쓰기 규칙:
 *   INDENT  - 섹션 내용 1단계, 전각공백 2개
 *   INDENT2 - 날짜 헤더 2단계, 전각공백 3개 (금요일 금주/차주 하위)
 *   INDENT3 - 티켓 행 3단계, 전각공백 4개 (날짜 헤더 하위)
 *
 * 날짜 형식: MM/DD(요일), 한 자리 월/일은 앞에 0 추가 (예: 02/07).
 *
 * 미완료 티켓 현황 규칙:
 *   조건: dueDate < 이번 주 월요일 AND status != DONE
 *   이슈키: Slack 링크 <url|[KEY]>
 *   제목: 인라인 백틱으로 감싸 색상 강조 `제목`
 *
 * 티켓 현황 규칙:
 *   금요일엔 [금주] / [차주] 헤더로 분리
 *   기한 초과 + 미완료 → bold
 *
 * 부재 & 재택 섹션:
 *   WorkStatusCode.UNKNOWN 마커 타입은 memberName에 "이름(부재타입)"이 이미 포함되어 있으므로 그대로 표시
 *   기타 타입(재택 등)은 memberName + "(" + workStatus.getDisplayName() + ")"
 *
 * 오늘 일정 섹션:
 *   위치 - 팀원 미완료 티켓 현황 다음, 주요 페이지 이전
 *   대상 - /일정등록 커맨드로 등록된 본인 당일 일정
 *   정렬 - 종일 일정 우선 → 시작시간 오름차순
 *   포맷 - [HH:mm-HH:mm] 제목 또는 [종일] 제목, URL 있으면 제목을 Slack 링크로 표시
 */
@Slf4j
public class DailyReportFormatter {
   ...
   *
   * 공지 한 줄 포맷 규칙 (우선순위 순):
   *   1. URL 있음: URL 링크가 최우선. type이 있으면 링크 텍스트(message)에만 강조 적용.
   *        bold: <url|*message*>, red: <url|`message`>, 미설정: <url|message>
   *   2. URL 없음: type에 따라 message에 직접 강조 적용.
   *        bold: *message*, red: `message`, 미설정: plain message
   */
   ...
   * URL이 있으면 Slack 링크로 감싸며 type 강조는 링크 텍스트(message)에만 적용한다.
  ...
  private String formatAnnouncementLine(AnnouncementItem a) {
    // type에 따른 message 강조 텍스트 생성 — bold: *message*, red: `message`, 미설정: message
    String displayText;
    if (a.isBold()) {
   ...
   * 날짜별 부재 & 재택 한 줄 포맷.
   *
   * WorkStatusCode.UNKNOWN 마커 타입(AbsenceTypeCode 파싱 결과)은
   * Collector에서 이미 memberName을 "이름(부재타입)" 형식으로 저장했으므로 그대로 표시한다.
   * 예: "홍길동(연차)", "김철수(오전 반차)"
   *
   * 기존 타입(REMOTE 등)은 memberName + "(" + displayName + ")" 형식으로 표시한다.
   * 예: "홍길동(재택)"
   */
  private String buildAbsenceRow(LocalDate day, Map<LocalDate, List<AbsenceItem>> byDate,
    ...
    String people = items.isEmpty()
        ...
        : items.stream()
        .map(a -> {
          // AbsenceTypeCode 파싱 항목: memberName이 "이름(부재타입)" 형식으로 저장되어 있으므로 그대로 사용
  ...
  /**
   * 미완료 티켓 현황 섹션 추가.
   * 조건: dueDate < 이번 주 월요일 AND status != DONE, 정렬: dueDate 오름차순.
   *
   * 포맷 예시:
   *   🚨 미완료 티켓 현황
   *     *02/17(화)*
   ...
   *
   * 이슈키는 Slack 링크 <url|[KEY]>로 클릭 가능, 제목은 인라인 백틱으로 회색 강조한다.
   */
  private void appendOverdueTicketSection(SlackBlockBuilder builder,
    ...
    for (Map.Entry<LocalDate, List<TicketItem>> entry : groups.entrySet()) {
      ...
      for (TicketItem t : entry.getValue()) {
        // 이슈키/제목 모두 백틱으로 회색 강조. 이슈키는 Slack 링크도 유지.
        // 주의: 백틱 내부에 <url|text>를 넣으면 링크 비활성화되므로, 백틱은 텍스트만 감싸고 링크 태그는 바깥에 둔다.
   ...
   *
   * 금요일이면 [금주] / [차주] 헤더로 분리:
   *   금주: dueDate <= 이번 금요일 (오늘)
   *   차주: dueDate > 이번 금요일 OR dueDate == null
   *
   * 금요일 아닌 경우 - 날짜 헤더 INDENT, 티켓 행 INDENT2
   * 금요일인 경우 - 금주/차주 헤더 INDENT, 날짜 헤더 INDENT2, 티켓 행 INDENT3
   */
  private void appendTicketSection(SlackBlockBuilder builder,
    ...
    if (!isFriday) {
      ...
      // 금요일: 금주/차주 분리. 한 주 = 월~일이므로 금주 끝은 이번 주 일요일
  ...
  /**
   * 날짜 그룹 맵을 순서대로 StringBuilder에 추가. 2800자 초과 시 자동으로 section 분할.
   *
   ...
   *
   * 표시 규칙 (우선순위 순):
   *   1. 기한 초과 + 미완료 (dueDate < baseDate AND status != DONE):
   *      이슈키/제목 모두 인라인 백틱(`...`)으로 감싸 회색 강조.
   *      이슈키는 Slack 링크도 유지 (<url|`[KEY]`>). bold 처리하지 않음.
   *   2. 당일 마감 + 미완료 (dueDate == baseDate AND status != DONE):
   *      이슈키/제목 모두 bold(*...*)로 강조. 이슈키 포함 전체 행에 적용.
   *   3. 일반: 링크 + 제목 plain 출력.
   *
   * @param t 티켓 아이템
  ...
  private String formatTicketLine(TicketItem t, LocalDate baseDate, String indent) {
    ...
    // 기한 초과 + 미완료: 백틱 처리 (DONE이면 이미 처리 완료이므로 대상 아님)
    ...
    if (isDueToday) {
      ...
      // 백틱 내부에 <url|text>를 넣으면 링크 비활성 → 링크 태그는 밖에, 백틱은 텍스트만 감싸기
      String issueLink = "<" + t.getUrl() + "|`[" + t.getIssueKey() + "]`>";
  ...
  private void appendTeamTicketSection(SlackBlockBuilder builder,
                                       ...
                                       LocalDate baseDate) {
    // Slack section 블록 사이에는 자동 여백이 생기므로, [금주]/[차주] + 전체 팀원을 하나의 section에 담아
    // 여백을 최소화한다. 2800자 초과 시 자동 분할하되 제목 없이 이어서 출력.
   ...
   * 위치: 팀원 미완료 티켓 현황 다음, 주요 페이지 이전.
   * todaySchedules가 null이거나 비어 있으면 "_없음_"으로 출력한다.
   *
   * 행 포맷:
   *   시간 지정: [HH:mm-HH:mm] 제목 (예: [09:00-10:00] 주간 회의)
   *   종일: [종일] 제목
   *   URL 있으면 제목 부분을 Slack 링크로 표시: [HH:mm-HH:mm] <url|제목>
   *
   * 정렬은 Collector에서 이미 완료 (종일 우선 → startTime 오름차순).
   *
   * @param builder Slack 블록 빌더
   * @param todaySchedules 오늘 일정 목록
   */
   ...
   * 시간 태그: [HH:mm-HH:mm] 또는 [종일]. 종료시간 없는 경우 시작시간만 [HH:mm].
   * 제목: URL 있으면 <url|제목>, 없으면 plain 텍스트.
   *
   * @param s 일정 항목
  ...
  /** LocalTime을 "HH:mm" 형식(항상 2자리)으로 변환한다. */
  ...
  /**
   * 날짜를 MM/DD(요일) 레이블로 변환 (한 자리 월/일 앞에 0 추가).
   * 예: 2026-02-07 → "02/07(토)"
   */
