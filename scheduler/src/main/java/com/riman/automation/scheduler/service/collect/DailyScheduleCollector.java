 ...
 *
 * 수집 전략:
 *   1. DynamoDB ScheduleEventMapping 테이블에서 slackUserId로 본인 이벤트 ID 목록을 조회
 *   2. Google Calendar listEvents로 오늘 하루 범위 이벤트를 조회
 *   3. DynamoDB eventId 집합과 Calendar 이벤트 ID를 교차 매칭하여 본인 이벤트만 추출
 *   4. 이벤트에서 endDateTime, url을 직접 파싱
 *
 * 데이터 소스 분리 이유:
 * DynamoDB에는 startDateTime만 저장되어 있고 endDateTime, url은 저장되지 않는다.
 * Google Calendar 이벤트가 원본이므로 Calendar API에서 직접 가져온다.
 *
 * URL 파싱 규칙:
 * worker ScheduleFacade.buildDescription()이 description 마지막에
 * "🔗 https://..." 형식으로 URL을 기록하므로 "🔗 " 접두사 라인에서 추출한다.
 *
 * 오늘 일정 기준: startDate(종일) 또는 startDateTime(시간 지정)이 baseDate(KST 오늘)인 것.
 * 정렬: 종일 일정(allDay=true) → 시간 지정 일정 (startTime 오름차순).
 *
 * 환경변수: SCHEDULE_MAPPING_TABLE 미설정 시 빈 목록 반환 (보고서 발송은 계속).
 */
@Slf4j
...
public class DailyScheduleCollector {
    ...
    for (TeamMember member : members) {
      ...
      try {
        ...
        for (String eventId : myEventIds) {
          ...
          if (event == null) continue; // 오늘 이벤트가 아니면 제외
        ...
        // 개별 팀원 실패는 빈 목록으로 처리 — 보고서 발송은 계속
      }
    }
  ...
  /**
   * Google Calendar Event를 ScheduleItem으로 변환.
   * startDate(종일) 또는 startDateTime(시간 지정) 기준으로 allDay, startTime, endTime을 설정한다.
   *
   ...
   * Google Calendar API가 반환하는 datetime은 "2026-03-08T09:00:00+09:00" 또는
   * "2026-03-08T00:00:00Z" 형식이다. Z(UTC) 형식이면 9시간을 더해 KST로 변환한다.
   */
  private LocalTime parseTime(String rfc3339) {
    if (rfc3339 == null || rfc3339.length() < 16) return null;
    try {
      // "HH:mm" 부분 추출: rfc3339 예시 "2026-03-08T09:00:00+09:00"
      String timePart = rfc3339.substring(11, 16);
   ...
   *
   * worker ScheduleFacade.buildDescription()이 생성하는 형식:
   *   📝 등록자: 홍길동 (@hong)
