 ...
 *
 * 시트 설명:
 *   전체 - 모든 카테고리 합산, 기한 오름차순 → 카테고리 순 정렬
 *   카테고리별 - 주문/회원/수당/포인트/ABO/RBO
 *
 * 컬럼:
 *   시작(startDate) - extendedProperties["jiraStartDate"] 또는 description "Start Date:" 파싱.
 *                      시작일 정보가 없는 과거 이벤트는 빈 칸 표시.
 *   중요도(priority) - description "Priority:" 파싱. 없으면 빈 칸.
 *
 * 상태 분류 기준:
 *   완료 - doneByCategory에 포함된 티켓
 *   이슈 - issuesByCategory에 포함된 티켓 (진행 중 [이슈] 태그)
 *   진행 - inProgressByCategory에 포함되고 이슈가 아닌 티켓
 */
@Slf4j
public class WeeklyExcelGenerator {
   ...
   * 정렬 기준: 기한(dueDate) 오름차순(null은 맨 뒤) → 카테고리 순 (CATEGORY_ORDER 기준).
   * 카테고리 컬럼 포함 (8번째 컬럼).
   */
   ...
   * 완료 → 이슈 → 진행 순으로 수집 후 기한 오름차순 → 카테고리 순으로 정렬한다.
   */
  ...
  /**
   * 티켓의 상태 레이블 결정 (전체 탭용).
   * issuesByCategory에 있으면 이슈, doneByCategory에 있으면 완료, 나머지는 진행.
   */
  ...
  private static class Styles {
    ...
    Styles(XSSFWorkbook wb) {
      ...
      done.setFillForegroundColor(new XSSFColor(new byte[]{(byte) 227, (byte) 252, (byte) 239}, null));
      ...
      issue.setFillForegroundColor(new XSSFColor(new byte[]{(byte) 255, (byte) 235, (byte) 230}, null));
