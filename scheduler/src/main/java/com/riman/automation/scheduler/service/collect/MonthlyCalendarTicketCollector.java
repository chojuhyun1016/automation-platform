 ...
 *
 * WeeklyCalendarTicketCollector와 대응 — 주간은 전주 완료/분기 진행중, 월간은 대상 월 완료/분기 진행중을 수집한다.
 *
 * 수집 전략 — 캘린더 2회 조회:
 *   1. 완료: monthStart~monthEnd 범위 중 Status=DONE인 것
 *   2. 진행중: quarterStart~quarterEnd 범위(분기 전체) 중 Status≠DONE이고
 *      startDate ≤ monthEnd(또는 startDate null)이며 [이슈] 태그가 없는 것
 *   3. 이슈: quarterStart~quarterEnd 범위(분기 전체) 중 [이슈] 태그 포함 미완료이고
 *      startDate ≤ monthEnd(또는 startDate null)인 것
 *
 * 진행중/이슈 startDate 필터 이유:
 * 분기 전체를 조회하면 대상 월 이후 시작 예정 티켓(startDate > monthEnd)이 포함되므로 제외한다.
 * startDate가 null인 이벤트는 시작일 정보가 없는 과거 이벤트이므로 필터를 적용하지 않고 포함한다.
 *
 * 공통 파싱 로직은 CalendarTicketParser에 위임한다.
 */
@Slf4j
...
public class MonthlyCalendarTicketCollector {
  ...
  /** Jira API 클라이언트 (nullable). null이면 상태 재검증 건너뜀 (degraded mode). */
  ...
  /** Jira 상태 재검증 — CalendarTicketParser에 위임. */
   ...
   * startDate가 null인 이벤트는 시작일 정보가 없는 과거 이벤트이므로 제외하지 않고 포함한다.
   *
