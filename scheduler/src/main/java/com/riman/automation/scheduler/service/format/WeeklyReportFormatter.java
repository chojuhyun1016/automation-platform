...
/**
 * 주간보고 Confluence Storage Format HTML 포맷터.
 *
 * 구조:
 *   [헤더 패널] 기간 / 분기 / 전체 건수
 ...
 *   ▼ [회원]  완료 1 / 진행 4
 *
 * 테이블 너비: data-layout="default" + colgroup px 단위 (% 는 Confluence에서 무시됨).
 *
 * 줄바꿈 방지: 날짜/이슈키 셀 내용을 span(display:inline-block)으로 감싼다.
 * white-space:nowrap, &#40; 엔티티, non-breaking hyphen 모두 Confluence가 무시하므로
 * inline-block span이 유일하게 동작하는 방법이다.
 */
public class WeeklyReportFormatter {

  /** "03/06(화)" 형식 — span으로 감싸서 줄바꿈 방지. */
  ...
  public String format(WeeklyReportData data) {
    ...
    // 첨부파일 섹션 — Confluence Cloud는 자동 표시 안 하므로 매크로로 명시
